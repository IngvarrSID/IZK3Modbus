        ...
        if (izkModbusGUI.getExecutor().getActiveCount() < 2) {
            if (izkModbusGUI.getChannelsBox().getSelectedIndex() != 5) {
                try {
                    ...
                    if (status.equals("Данные получены") || status.equals("Нулевой период")) {
                        query.queryMain();
                        ...
                        izkModbusGUI.getDensityExField().setText(String.format("Плотность ком.: %.1f кг/м²", query.getDensityEx()));
